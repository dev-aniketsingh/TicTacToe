...
float evaluate(char board[4][4], char who)
{
 ...
 //REWARD  VARIABLES.  (Play with these variables)  
 float VERY_VERY_LOW = 5.0,SLIGHTLY_LOW = 30.0,BETTER_THAN_SLIGHTLY_LOW = 60.0, LOW_REWARD = 100.0, MORE_THAN_LOW = 250.0, MID_REWARD = 400.0, HIGH_REWARD = 600.0;
//PENALITY VARIABLES (Play with these varibles)
 float VERY_VERY_LOW_PEN = 3.0,SLIGHTLY_LOW_PEN = 5.0,BETTER_THAN_SLIGHTLY_LOW_PEN = 20.0, LOW_REWARD_PEN = 80.0, MORE_THAN_LOW_PEN = 200.0, MID_LEVEL_PEN = 400.0, HIGH_PEN = 700.0;
 ...
 for(int i = 0; i < 4; i++) {
  ...
  if(penalityValue > rewardValue) {
  	result = 10;   //can play with this value
  } 
  if (winner(board, who)) 
  	result = 10000;
